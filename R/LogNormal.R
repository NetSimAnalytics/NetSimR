...
#' @return The mean of the claim severity capped at \code{cap} with a LogNormal distribution with parameters \code{mu} and \code{sigma}. A negative \code{cap} or a non-positive \code{sigma} is an error; \code{NA} values give \code{NA}.
#' @export
#' @examples
...
LNormCappedMean<- function(cap,mu,sigma){
  ...
  exp(mu + 0.5 * sigma * sigma + pnorm(logCap, mean = mu + sigma * sigma, sd = sigma, log.p = TRUE)) + capTerm
}
...
PureIBNRLNorm <- function(IncDate, ExpDate, ValDate, mu, sigma){
  check_positive(sigma = sigma)
  pure_ibnr_exposure(IncDate, ExpDate, ValDate, function(delay) LNormCappedMean(delay,mu,sigma))
}
